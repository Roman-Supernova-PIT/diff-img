import nvtx
with nvtx.annotate( "Importing os" , color="yellow"):
    import os
with nvtx.annotate( "Importing sfft", color="yellow" ):
    import sfft
...
with nvtx.annotate( "Importing astropy", color="yellow" ):
...
with nvtx.annotate( "Initial header setup", color="orange" ):
...
with nvtx.annotate("sky_subtract", color="green"):
...
with nvtx.annotate("swarp", color="red"):
...
with nvtx.annotate("align_psf", color="blue"):
...
with nvtx.annotate("convolve_get", color="blue"):
...
with nvtx.annotate("convolve_get", color="purple"):
...
with nvtx.annotate("Cutouts", color="red"):
...
with nvtx.annotate("sfft_subtraction", color="red"):
...
with nvtx.annotate("zero_background", color="green"):
...
colors = [ 'white', 'yellow' ]
for i in range(2):
    with nvtx.annotate("sfft", color="white"):
...
with nvtx.annotate("noise_decorrelation", color="white"):
    PixA_PSF_lREF = fits.getdata(FITS_PSF_lREF, ext=0).T
    PixA_PSF_lSCI = fits.getdata(FITS_PSF_lSCI, ext=0).T
...
with nvtx.annotate("snr", color="white"):
    GAIN = 1.0  
