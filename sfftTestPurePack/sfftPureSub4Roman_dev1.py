...
GDIR = '/hack_team_01/leiworkflow/diff-img/sfftTestPurePack'  # curiosity
...
def WorkFlow_NVTX(refname, sciname, input_dir, output_dir, aux_dir, config_file):
    ...
    def run_resample(FITS_obj, FITS_targ):
        ...
        XX_proj_GPU, YY_proj_GPU = CR.projection_sip(hdr_obj=hdr_obj, hdr_targ=hdr_targ, Nsamp=1024, RANDOM_SEED=10086)
    ...
    with nvtx.annotate("Step0b_resamp_main", color="#8337EC"):
        PixA_resamp1 = run_resample(FITS_obj=FITS_oSCI, FITS_targ=FITS_REF)
        PixA_resamp2 = run_resample(FITS_obj=FITS_oSCI_DMASK, FITS_targ=FITS_REF_DMASK)
